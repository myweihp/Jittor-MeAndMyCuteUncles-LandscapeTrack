...
#     return x / x.sum(dim, keepdims=True).expand(shape)
...
class Attention(nn.Module):
    ...
    def execute(self, x, y=None):
        ...
        # theta = theta.reshape(b, self.ch // 8, -1)
        # phi = phi.reshape(b, self.ch // 8, -1)
        # g = g.reshape(b, self.ch // 2, -1)
        # theta = jt.flatten(theta, -2, -1)
        # phi = jt.flatten(phi, -2, -1)
        
        # Matmul and softmax to get attention maps
        # tmp = nn.bmm(theta.t(), phi)
        # tmp = nn.bmm(theta.transpose(1, 2), phi)

        # print(theta.dtype, phi.dtype, g.dtype, tmp.dtype)
        # print(theta.shape, phi.shape, g.shape, tmp.shape)
        # beta = nn.softmax(tmp, -1)
        # beta = nn.softmax(nn.bmm(theta.t(), phi), -1)
        # beta = nn.softmax(nn.bmm(theta.t(), phi), -1)
