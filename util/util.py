...
def masktorgb(x):
    mask = np.zeros((x.shape[0], 3, x.shape[2], x.shape[3]), dtype=np.uint8)
...
def tensor2im(image_tensor, imtype=np.uint8, normalize=True, tile=False):
    ...
    image_numpy = image_tensor.detach().float().data
    if normalize:
        image_numpy = (np.transpose(image_numpy, (1, 2, 0)) + 1) / 2.0 * 255.0
        # image_numpy = (np.transpose(image_numpy, (1, 2, 0)) + 1) / 2.0 * 1.0
    else:
        image_numpy = np.transpose(image_numpy, (1, 2, 0)) * 255.0
        # image_numpy = np.transpose(image_numpy, (1, 2, 0)) * 1.0
